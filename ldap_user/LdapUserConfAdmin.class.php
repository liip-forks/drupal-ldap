...
class LdapUserConfAdmin extends LdapUserConf {
  ...
  protected function setTranslatableProperties() {
    ...
    $values['ldapEntryProvisionTriggersOptions'] = array(
      ...
      LDAP_USER_LDAP_ENTRY_DELETE_ON_USER_DELETE => t('Delete LDAP entry when the corresponding Drupal Account is deleted.'),
    );
  ...
  public function drupalForm() {
   // // temp_out dpm('this in drupal form'); // temp_out dpm($this->ldapUserSynchMappings['uiuc_ad']);
    ...
    $form['basic_to_drupal']['orphanedDrupalAcctBehavior'] = array(
      '#type' => 'radios',
      '#title' => t('[Not Implemented] Action to perform on Drupal account that no longer has a corresponding LDAP entry'),
      '#required' => 0,
    ...
    if (isset($this->ldapUserSynchMappings)) {
     ...
     // foreach ($this->ldapUserSynchMappings as $sid => $mappings) {
      foreach ($this->ldapUserSynchMappings as $synch_direction => $provision_servers) { // there is only 1 allowed, but designed for multiple provision servers
        // $synch_direction = LDAP_USER_PROV_DIRECTION_TO_DRUPAL_USER or LDAP_USER_PROV_DIRECTION_TO_LDAP_ENTRY
...
**/
      ...
      if ($row_mappings['configurable_to_drupal'] && $row_mappings['ldap_attr'] && $row_mappings['user_attr']) {
        ...
        foreach ($synchEvents as $prov_event => $discard) {
          ...
          if (isset($values[$input_name]) && $values[$input_name]) {
            $mappings[$direction][$sid][$key]['prov_events'][] = $prov_event;
          }
        }
      }
    }
 ...
 //   if ($mapping['ldap_attr'] == '[objectclass:1]') {
   //   dpm("provisionEventConfigurable:prov_event=$prov_event,configurable=$configurable"); dpm($mapping);
 //   }
   return $configurable;
  }
}
